	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (greensService.checkGreensname(greensname, restid)) {
			if (greensService.addGreens(greens,restid)) {
				out.println(
						"<script>if(confirm('新菜上架成功，是否继续添加')){location='greens_add.jsp'}else{location='rest_index.jsp'}</script>");
			} else {
			...
			if (greensService.updateGreens(oldgreens, greens)) {
				out.println(
						"<script>if(confirm('新菜更新成功，是否继续添加')){location='greens_add.jsp'}else{location='rest_index.jsp'}</script>");
			} else {
